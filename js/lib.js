...
_.Module.define('gb/widget/adroll', function(require, exports, module) {
  ...
  return function(options) {
    ...
    this.nav = options.nav || '.cmp_slider_nav';
    this.navtag = options.navtag || 'i';
    this.pic = options.pic || '.cmp_slider_wp';
    ...
    this.init = function() {
      ...
      var el = this.el,
        _this = this;
      var nav = [];
      $(_this.pic, el).children().each(function(i) {
        nav.push('<' + _this.navtag + '>' + (i + 1) + '</' + _this.navtag + '>');
      });

      $(_this.nav, el).html(nav.join(''))
        .on('mouseover', _this.navtag, function() {
          var self = $(this);
          _this.playIndex(self.index());
          _this.pause();
        });
      ...
      }).css('position', 'relative').children().css({
        top: '0',
        left: '0',
        'position': 'absolute',
        'display': 'none'
    ...
    this.playIndex = function(_index) {
      ...
      var ishow = arguments[1] || false;
      var imgs = $(this.pic, this.el).children(),tg = imgs.eq(_index);
      if(ishow){
         tg.show();
       }else{
         tg.stop(false, true).fadeIn(500);
       }
       tg.addClass('active').siblings().removeClass('active');
      $(this.nav, this.el).children().eq(_index).addClass('on').siblings().removeClass('on');
      
      if (this.total > 1) {
        imgs.eq(this.currentIndex).stop(false, true).fadeOut(400);
      }
